function lab_transform_BinarySearch_convhull_new_dim2(rawMat)
    tic %time start
    ...
    [eigenVector,score,eigenvalue] = princomp(rawMat); %PCA 2dim->2dim (for principal component)
    %transMatrix(:,1:2) = eigenVector(:,1:2);
    %color_mat = rawMat * transMatrix;
    color_mat = score(:,1:2);
    %PCA的三個軸:e1,e2,e3
    e1 = eigenVector(1:2,1);
    e2 = eigenVector(1:2,2);
